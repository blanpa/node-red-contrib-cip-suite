...
module.exports = function (RED: any) {
  function CipSyncNode(this: any, config: any) {
     ...
     */
    function buildTimeSyncRequest(service: number, instance: number, attribute?: number): Buffer {
      const pathParts: number[] = [0x20, TIME_SYNC_CLASS, 0x24, instance];
     ...
     */
    async function sendCIPRequest(req: Buffer): Promise<{ status: number; data: Buffer }> {
     ...
     */
    async function readAttribute(attribute: number): Promise<{ status: number; data: Buffer }> {
     ...
     */
    async function readPTPStatus(): Promise<any> {
      ...
      try {
        ...
        if (status === 0 && data.length > 0) {
          ...
          let off = 0;
          if (data.length >= off + 1) { result.ptpEnable = data.readUInt8(off) !== 0; off += 1; }
          if (data.length >= off + 1) { result.isSynchronized = data.readUInt8(off) !== 0; off += 1; }
      ...
      try {
        ...
        if (status === 0) result.ptpEnable = data.length > 0 ? data.readUInt8(0) !== 0 : null;
      } catch (_) { result.ptpEnable = null; }

      try {
        const { status, data } = await readAttribute(TS_ATTR.IS_SYNCHRONIZED);
        if (status === 0) result.isSynchronized = data.length > 0 ? data.readUInt8(0) !== 0 : null;
      } catch (_) { result.isSynchronized = null; }
      ...
      } catch (_) { result.offsetFromMaster = null; }
      ...
      } catch (_) { result.meanPathDelay = null; }
      ...
      } catch (_) { result.grandmaster = null; }
      ...
      } catch (_) { result.numberOfPorts = null; }
      ...
      } catch (_) { result.stepsRemoved = null; }
      ...
      } catch (_) { result.domainNumber = null; }
      ...
      try {
        ...
        if (status === 0 && data.length >= 2) {
          ...
          if (clockType & 0x01) result.clockTypeText.push("Ordinary Clock");
          if (clockType & 0x02) result.clockTypeText.push("Boundary Clock");
          if (clockType & 0x04) result.clockTypeText.push("Peer-to-Peer Transparent Clock");
          if (clockType & 0x08) result.clockTypeText.push("End-to-End Transparent Clock");
          if (clockType & 0x80) result.clockTypeText.push("Slave Only");
        }
      } catch (_) { result.clockType = null; }
      ...
      } catch (_) { result.timeSource = null; }
     ...
     */
    async function setPTPEnable(enable: boolean): Promise<any> {
    ...
    async function doRead(): Promise<void> {
      if (!node.endpoint.connected || node._busy) return;
      node._busy = true;
      try {
        const { result, elapsed } = await withTiming(() => readPTPStatus());
