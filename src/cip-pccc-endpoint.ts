...
import { buildCIPPcccMessage } from "./pccc-utils";
import { CipPcccEndpointConfig } from "./types";
...
module.exports = function (RED: any) {
  function CipPcccEndpointNode(this: any, config: CipPcccEndpointConfig) {
    ...
    node._socket = null as net.Socket | null;
    ...
    node._pendingRequests = new Map<number, PendingRequest>(); // keyed by TNS

    // ── User registration ──

    node.register = function (userNode: any) {
    ...
    node.deregister = function (userNode: any) {
      node._users.delete(userNode);
    };

    node._broadcast = function (event: string, data?: any) {
    ...
    function buildRegisterSessionData(): Buffer {
    ...
    function buildSendRRDataPacket(cipMessage: Buffer, sessionHandle: number): Buffer {
      ...
      // Timeout
      data.writeUInt16LE(10, offset); offset += 2;
      // Item count
      data.writeUInt16LE(2, offset); offset += 2;
      // Item 1: Null Address
      data.writeUInt16LE(0x0000, offset); offset += 2; // type
      data.writeUInt16LE(0, offset); offset += 2;      // length
      // Item 2: UCMM Data
      data.writeUInt16LE(0x00b2, offset); offset += 2; // type
      data.writeUInt16LE(cipMessage.length, offset); offset += 2; // length
      cipMessage.copy(data, offset); offset += cipMessage.length;
      ...
      while (node._rxBuffer.length >= ENCAP_HEADER_LENGTH) {
        ...
        if (command === ENCAP_CMD_REGISTER_SESSION) {
          if (status !== 0) {
            ...
            if (node._socket) node._socket.destroy();
            return;
          }
    ...
    node._handleSendRRDataResponse = function (packet: Buffer) {
      ...
      const itemCount = packet.readUInt16LE(offset); offset += 2;

      // Find UCMM Data item (type 0x00B2)
      for (let i = 0; i < itemCount; i++) {
        if (offset + 4 > packet.length) return;
        const typeId = packet.readUInt16LE(offset); offset += 2;
        const itemLength = packet.readUInt16LE(offset); offset += 2;
    ...
    node._handleCIPReply = function (cipReply: Buffer) {
      // CIP reply format:
      // service(1) + reserved(1) + generalStatus(1) + addStatusSize(1) + [addStatus] + data
      if (cipReply.length < 4) return;
      ...
      if (cipReply.length <= dataOffset) return;
      ...
      if (responseData.length < pcccOffset + 4) return;
    ...
    node._rejectOldest = function (err: Error) {
    ...
    node._rejectAllPending = function (err: Error) {
    ...
    node.sendPCCC = function (
      ...
      return new Promise<Buffer>((resolve, reject) => {
        ...
        const cipMessage = buildCIPPcccMessage(cmd, fnc, data, tns);
