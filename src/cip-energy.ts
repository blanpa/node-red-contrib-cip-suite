...
const ENERGY_MODE_NAMES: Record<number, string> = {
  0: "Normal",
  1: "Energy Saving",
  2: "Energy Off",
  3: "Paused",
...
const ENERGY_TYPE_NAMES: Record<number, string> = {
  0: "Non-Specific",
  1: "Electrical AC",
  2: "Electrical DC",
  3: "Pneumatic",
...
module.exports = function (RED: any) {
  function CipEnergyNode(this: any, config: any) {
     ...
     */
    async function readAttr(classId: number, instance: number, attr: number): Promise<any> {
     ...
     */
    async function readEnergyStatus(): Promise<any> {
      const result: any = { instance: node._instance, timestamp: Date.now() };
      ...
      } catch (_) { result.energyType = null; }
      ...
      } catch (_) { result.energyMode = null; }
      ...
      } catch (_) { result.nominalPower = null; }
     ...
     */
    async function readElectricalEnergy(): Promise<any> {
      const result: any = { instance: node._instance, type: "electrical", timestamp: Date.now() };
     ...
     * Set energy mode.
     */
    async function setEnergyMode(mode: number): Promise<any> {
    ...
    async function doRead(): Promise<void> {
      ...
      node._busy = true;
      try {
        const { result, elapsed } = await withTiming(() => readEnergyStatus());
    ...
    node.on("input", async function (msg: any) {
      ...
      try {
        ...
        switch (command) {
          ...
          case "electrical": {
            const { result, elapsed } = await withTiming(() => readElectricalEnergy());
            result.elapsed = elapsed;
            msg.payload = result;
            break;
          }
