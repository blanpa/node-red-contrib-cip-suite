...
import { CipDiscoverConfig, DiscoveredDevice, NodeStatus } from "./types";
 ...
 */
function buildListIdentityPacket(): Buffer {
 ...
 */
function parseListIdentityResponse(data: Buffer, remoteAddress: string): DiscoveredDevice | null {
  try {
    if (data.length < ENCAP_HEADER_SIZE + 6) return null;
...
module.exports = function (RED: any) {
  function CipDiscoverNode(this: any, config: CipDiscoverConfig & { id: string; name: string }) {
    RED.nodes.createNode(this, config);
    const node = this;

    node.broadcastAddress = config.broadcastAddress || "255.255.255.255";
    node.timeout = parseInt(config.timeout as any, 10) || 3000;
