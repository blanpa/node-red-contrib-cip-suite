...
module.exports = function (RED: any) {
  function CipMotionNode(this: any, config: any) {
    ...
    function buildCIPPath(classId: number, instance: number, attribute?: number): Buffer {
      const parts: number[] = [0x20, classId, 0x24, instance];
    ...
    function buildRequest(service: number, path: Buffer, data?: Buffer): Buffer {
    ...
    async function sendRequest(req: Buffer): Promise<{ status: number; data: Buffer }> {
     ...
     */
    async function readAxisStatus(): Promise<any> {
      const result: any = {
      ...
      // Read key attributes
      const attrs = [
        { attr: AXIS_ATTR.AXIS_STATE, name: "axisState", parser: (d: Buffer) => d.readUInt8(0) },
        { attr: AXIS_ATTR.ACTUAL_POSITION, name: "actualPosition", parser: (d: Buffer) => d.readFloatLE(0) },
        { attr: AXIS_ATTR.ACTUAL_VELOCITY, name: "actualVelocity", parser: (d: Buffer) => d.readFloatLE(0) },
        { attr: AXIS_ATTR.COMMAND_POSITION, name: "commandPosition", parser: (d: Buffer) => d.readFloatLE(0) },
        { attr: AXIS_ATTR.COMMAND_VELOCITY, name: "commandVelocity", parser: (d: Buffer) => d.readFloatLE(0) },
        { attr: AXIS_ATTR.ACTUAL_TORQUE, name: "actualTorque", parser: (d: Buffer) => d.readFloatLE(0) },
        { attr: AXIS_ATTR.POSITION_ERROR, name: "positionError", parser: (d: Buffer) => d.readFloatLE(0) },
        { attr: AXIS_ATTR.AXIS_FAULT, name: "axisFault", parser: (d: Buffer) => d.readUInt32LE(0) },
      ];
     ...
     */
    async function executeMotionCommand(cmd: string, params: any): Promise<any> {
      const result: any = { command: cmd, axisInstance: node._axisInstance, timestamp: Date.now() };
    ...
    async function doStatusPoll(): Promise<void> {
    ...
    node.on("input", async function (msg: any) {
      ...
      try {
        ...
        if (command === "status") {
          const { result, elapsed } = await withTiming(() => readAxisStatus());
          result.elapsed = elapsed;
          msg.payload = result;
        } else {
          const { result, elapsed } = await withTiming(() =>
            executeMotionCommand(command, msg.payload || {})
          );
          result.elapsed = elapsed;
          msg.payload = result;
        }
