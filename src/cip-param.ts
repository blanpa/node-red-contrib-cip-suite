...
const DATA_TYPE_SIZE: Record<number, number> = {
  0xc1: 1,  // BOOL
  0xc2: 1,  // SINT
  0xc3: 2,  // INT
  0xc4: 4,  // DINT
...
module.exports = function (RED: any) {
  function CipParamNode(this: any, config: any) {
    ...
    function buildReq(service: number, path: Buffer, data?: Buffer): Buffer {
      ...
      path.copy(req, 2);
      if (d.length > 0) d.copy(req, 2 + path.length);
      return req;
    }
     ...
     */
    function parseValue(data: Buffer, dataType: number): any {
     ...
     */
    function encodeValue(value: any, dataType: number): Buffer {
      const size = DATA_TYPE_SIZE[dataType] || 4;
      const buf = Buffer.alloc(size);
      switch (dataType) {
        case 0xc1: buf.writeUInt8(value ? 1 : 0, 0); break;
        case 0xc2: buf.writeInt8(Number(value), 0); break;
        case 0xc3: buf.writeInt16LE(Number(value), 0); break;
        case 0xc4: buf.writeInt32LE(Number(value), 0); break;
        case 0xc6: buf.writeUInt8(Number(value), 0); break;
        case 0xc7: buf.writeUInt16LE(Number(value), 0); break;
        case 0xc8: buf.writeUInt32LE(Number(value), 0); break;
        case 0xca: buf.writeFloatLE(Number(value), 0); break;
        case 0xcb: buf.writeDoubleLE(Number(value), 0); break;
     ...
     */
    function parseShortString(data: Buffer, offset: number): { str: string; bytesConsumed: number } {
      if (offset >= data.length) return { str: "", bytesConsumed: 0 };
     ...
     */
    async function readParameter(instance: number): Promise<any> {
      const param: any = { instance, timestamp: Date.now() };
      ...
      try {
        ...
        if (status === 0) param.valueRaw = data;
      } catch (_) {}
      ...
      if (param.valueRaw && param.dataType) {
        ...
        // Best guess
        if (param.valueRaw.length >= 4) param.value = param.valueRaw.readFloatLE(0);
        else if (param.valueRaw.length >= 2) param.value = param.valueRaw.readInt16LE(0);
        else param.value = param.valueRaw.readUInt8(0);
      }
      ...
      try {
        ...
        const { status, data } = await sendReq(buildReq(0x0e, p));
        if (status === 0 && data.length >= 1) param.dataSize = data.readUInt8(0);
      } catch (_) {}
      ...
      if (param.supportsScaling) {
        try {
          ...
          if (ms === 0 && md.length >= 2) param.scalingMultiplier = md.readUInt16LE(0);
          ...
          if (ds === 0 && dd.length >= 2) param.scalingDivisor = dd.readUInt16LE(0);
          ...
          if (os === 0 && od.length >= 2) param.scalingOffset = od.readInt16LE(0);
     ...
     */
    async function writeParameter(instance: number, value: any, dataType?: number): Promise<any> {
      ...
      if (!dataType) dataType = 0xc4; // default DINT
      ...
      return {
        ...
        value,
        status: resp.status,
        statusText: cipStatusText(resp.status),
        timestamp: Date.now(),
      };
    }
     ...
     */
    async function discoverParameters(): Promise<any[]> {
      const params: any[] = [];
    ...
    node.on("input", async function (msg: any) {
      ...
      if (node._busy) { node.warn("Request in progress"); return; }
      ...
      try {
        ...
        switch (command) {
          case "read": {
            ...
            const { result, elapsed } = await withTiming(() => readParameter(instance));
          ...
          case "write": {
            if (!instance) throw new Error("No parameter instance specified");
            ...
            const { result, elapsed } = await withTiming(() =>
              writeParameter(instance, value, dataType)
            );
            result.elapsed = elapsed;
            msg.payload = result;
            node.status({
          ...
          case "discover":
          case "list": {
            const { result, elapsed } = await withTiming(() => discoverParameters());
            msg.payload = {
              success: true,
              command: "discover",
              parameters: result,
