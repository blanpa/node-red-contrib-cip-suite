...
module.exports = function (RED) {
    function CipSecurityNode(config) {
         ...
         */
        function buildSecurityObjectRequest(service, instance, attribute) {
            const pathParts = [0x20, CIP_SECURITY_CLASS, 0x24, instance];
         ...
         */
        function parseSecurityAttributes(data) {
            ...
            try {
                ...
                if (data.length >= off + 1) {
                    attrs.state = data.readUInt8(off);
                    off += 1;
                }
                if (data.length >= off + 2) {
                    attrs.securityProfiles = data.readUInt16LE(off);
                    off += 2;
                }
                if (data.length >= off + 1) {
                ...
                attrs.profiles = [];
                if (attrs.securityProfiles & 0x01)
                    attrs.profiles.push("EtherNet/IP Integrity");
                if (attrs.securityProfiles & 0x02)
                    attrs.profiles.push("EtherNet/IP Confidentiality");
                if (attrs.securityProfiles & 0x04)
                    attrs.profiles.push("CIP Authorization");
                if (attrs.securityProfiles & 0x08)
                    attrs.profiles.push("CIP User Authentication");
                if (attrs.securityProfiles & 0x10)
                    attrs.profiles.push("Resource-Constrained CIP Security");
            }
            catch (_) {
...
//# sourceMappingURL=cip-security.js.map