...
module.exports = function (RED) {
    function CipRawNode(config) {
         ...
         */
        function buildCIPPath(classId, instanceId, attributeId) {
            const segments = [];
            // Class segment
            if (classId <= 0xff) {
         ...
         */
        function buildCIPRequest(service, path, data) {
         ...
         */
        function buildMultipleServicePacket(requests) {
            ...
            let offset = 0;
            // Service
            packet.writeUInt8(types_1.CIPService.MULTIPLE_SERVICE_PACKET, offset++);
            // Path size in words
         ...
         */
        function parseMultipleServiceResponse(raw) {
            if (!raw || raw.length < 6)
                return [];
        ...
        node.on("input", async function (msg) {
            ...
            try {
                ...
                if (Array.isArray(msg.requests) && msg.requests.length > 0) {
                    ...
                    const { result: rawResponse, elapsed } = await (0, utils_1.withTiming)(() => new Promise((resolve, reject) => {
                        controller.write_cip(mspPacket, useConnected, 10, (err, data) => {
                            if (err)
                                reject(err);
                            else
                                resolve(data);
                    ...
                    msg.payload = {
                        success: true,
                        service: types_1.CIPService.MULTIPLE_SERVICE_PACKET,
                ...
                // Send via write_cip
                const { result: rawResponse, elapsed } = await (0, utils_1.withTiming)(() => new Promise((resolve, reject) => {
                    controller.write_cip(reqBuf, useConnected, 10, (err, data) => {
                        if (err)
                            reject(err);
                        else
                            resolve(data);
...
//# sourceMappingURL=cip-raw.js.map